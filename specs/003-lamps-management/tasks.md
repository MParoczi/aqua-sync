...
- [ ] T007 Register `IDeviceScanner` and `ILampService` singletons in `AquaSync.App/App.xaml.cs`: add `services.AddSingleton<IDeviceScanner, DeviceScanner>()` and `services.AddSingleton<ILampService, LampService>()` to the `ConfigureServices` method; add required `using` directives for Chihiros types
...
- [ ] T016 [P] [US3] Create `LampDetailPage.xaml.cs` in `AquaSync.App/Views/LampDetailPage.xaml.cs`: minimal code-behind; constructor receives `LampDetailViewModel viewModel` via DI and assigns `DataContext = viewModel`; expose `ViewModel` property for `x:Bind`
